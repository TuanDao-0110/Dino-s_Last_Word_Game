...
describe("testing GET:api/user/", () => {
    test("get token error", async () => {
        const result = await api.get("/api/user").expect(401).expect(helps_1.Content_Type, helps_1.Content_Text);
...
describe("testing login ", () => {
    ...
    beforeAll(async () => {
        ...
        let userCredential = await (0, auth_1.signInWithEmailAndPassword)(auth, email, password);
        const user = userCredential.user;
        expect(user.email).toEqual(email);
        token = await user.getIdToken();
    });
    test("GET:api/user/ status code 200", async () => {
        await api.get("/api/user").set("Authorization", `Bearer ${token}`).expect(200).expect(helps_1.Content_Type, helps_1.Content_ApplicationJson);
    });
