...
describe("testing GET:/api/word/:category", () => {
  ...
  test("get status code 400", async () => {
    const result = await api.get(`${url}wrongcategory`).expect(400).expect(Content_Type, Content_Text);
    expect(result.text).toContain(Message.Invalid_Category_Select);
  });
  test("get status code 200 with all word in animals", async () => {
    await api.get(`${url}animals`).expect(200).expect(Content_Type, Content_ApplicationJson);
  });
  test("get status code 200 with all word in countries", async () => {
    await api.get(`${url}countries`).expect(200).expect(Content_Type, Content_ApplicationJson);
  });
  test("get status code 200 with all word in fruits", async () => {
    await api.get(`${url}fruits`).expect(200).expect(Content_Type, Content_ApplicationJson);
  });
});
describe("testing POST:/api/word/", () => {
  const url = "/api/word/";
  test("Status code 400 with invalid new word", async () => {
    const result = await api.post(`${url}`).send({ newword: 4, category: "wrong" }).expect(400).expect(Content_Type, Content_Text);
    expect(result.text).toContain(Message.Invalid_New_Word);
  });
  test("Status code 400 with invalid category", async () => {
    const result = await api.post(`${url}`).send({ newword: "newword", category: "eas" }).expect(400).expect(Content_Type, Content_Text);
    expect(result.text).toContain(Message.Invalid_Category_Select);
  });
  test("status code 201 once post successful vs code 200 when we get that word", async () => {
    let result = await api.post(`${url}`).send({ newword: "testing", category: "sports" }).expect(201).expect(Content_Type, Content_ApplicationJson);
    expect(result.body.msg).toContain(SuccessMessage.AddSuccess);
    result = await api.get(`${url}`).expect(200).expect(Content_Type, Content_ApplicationJson);
